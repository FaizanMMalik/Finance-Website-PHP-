...
<?php
       ...
       {
        $query = "select * from workersalary where id='$_POST[id]'";
       }
       else
       {
        $query = "select * from workersalary where id='$_GET[id]'";
       }
      ...
      if (isset($_POST['submit'])) {
        $sql = "UPDATE `workersalary` SET `salary`='$_POST[salary]',`date`='$_POST[date]',`detail`='$_POST[detail]' WHERE id='$_POST[id]'";
...
<div>
 <br>
    ...
    <form action="workersalaryupdate.php?id=<?php echo $_GET['id']?>" method="post">
    <div hidden>
      <label>id</label>
    ...
    padding: 12px 20px;
    margin: 8px 0;
    box-sizing: border-box;" required value="<?php echo $row['date'];?>">
      </div>
      <div>
    ...
    <div>
        ...
        <input type="text" name="detail" id="detail"   placeholder="Detail" required value="<?php echo $row['detail'];?>">
